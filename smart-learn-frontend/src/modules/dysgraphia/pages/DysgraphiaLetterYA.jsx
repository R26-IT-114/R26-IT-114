...
import yaGuideImage from '../../../assets/images/letter-ya-guide.png';
...
// Guide path for letter ය — adjust coordinates as needed
const YA_GUIDE_PATH =
  'M 200 220 C 260 140 380 120 420 200 C 460 300 380 380 300 380 C 220 380 180 300 200 220';

const START_MARKER = { x: 200, y: 220 };
const END_MARKER = { x: 300, y: 380 };
...
const DysgraphiaLetterYA = () => {
  ...
  // Guided animation (same behavior)
  useEffect(() => {
  ...
  // Render (mostly identical to TA but using YA constants)
  return (
    <main className='dg-shell dg-theme-ta dg-theme-ya'>
                ...
                <>
                  {!practiceBlind && !thirdPreviewVisible && (
                    <image
                      ...
                      className='dg-letter-guide-image'
                    />
                  )}
                  ...
                  {thirdPreviewVisible && (
                    <path d={YA_GUIDE_PATH} fill='none' stroke='rgba(255,255,255,0.95)' strokeWidth='40' strokeLinecap='round' strokeLinejoin='round' style={{ filter: 'drop-shadow(0 0 10px rgba(255,255,255,0.35))' }} />
                  )}
                  ...
                  {showGuide && !drawingMode && (
                    <>
                      <circle cx={nodesDeployed ? START_MARKER.x : originPoint.x} cy={nodesDeployed ? START_MARKER.y : originPoint.y} r='22' className={`dg-node dg-node-start ${nodesDeployed ? 'dg-deployed' : ''}`} />
                      <circle cx={nodesDeployed ? END_MARKER.x : originPoint.x} cy={nodesDeployed ? END_MARKER.y : originPoint.y} r='22' className={`dg-node dg-node-end ${nodesDeployed ? 'dg-deployed' : ''}`} />
                    </>
                  )}
