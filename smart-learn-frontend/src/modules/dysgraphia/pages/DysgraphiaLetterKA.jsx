...
const KA_GUIDE_PATH =
  'M 70.5 180.0 A 50.8 30.0 0 1 0 70.5 240.0 A 50.8 30.0 0 1 0 70.5 180.0 M 70.5 180.0 C 179.8 180.0 181.5 229.0 163.0 240.0 C 504.3 240.0 450.1 420.0 310.9 420.0 C 254.5 420.0 213.6 393.1 213.6 360.0 C 213.6 393.1 172.8 420.2 116.3 420.0 C 44.7 419.8 3.7 383.6 39.5 335.3 L 5.1 340.0 C 35.3 315.0 90.1 273.2 163.0 240.0 C 235.8 206.8 326.0 179.8 422.5 180.0 C 715.9 180.5 689.6 420.0 440.1 420.0';

const START_MARKER = { x: 70.5, y: 180.0 };
const END_MARKER = { x: 440.1, y: 420.0 };
...
// ──────────────────────────────────────────────────────
const KA_STAR_COLORS = ['#ffffff','#ffe4b5','#add8e6','#ffcccb','#b0e0e6','#fff176','#e0b0ff'];
const KaStarField = () => {
  const stars = Array.from({ length: 160 }, (_, i) => ({
    ...
    color: KA_STAR_COLORS[Math.floor(Math.random() * KA_STAR_COLORS.length)],
  }));
  return (
...
const DysgraphiaLetterKA = () => {
  ...
  const submitCanvasForEvaluation = async () => {
    ...
    try {
      ...
      setEvalResult(data);
      // Validate: check if the returned Sinhala letter matches "ක"
      const isCorrect = data?.predictions?.[0]?.sinhala === "ක" || data?.prediction?.sinhala === "ක";
  ...
  // ---------- Render ----------
  return (
    <main className='dg-shell dg-theme-ka'>
      <KaSpaceBackground />
        ...
        <div className='dg-canvas-wrap'>
          {!drawingWithCanvas ? (
            ...
            <div className='dg-practice-wrap' style={{ width: '100%', height: '100%' }}>
              <h3>✍️ දැන් “ක” අක්ෂරය ඔබම අඳින්න</h3>
