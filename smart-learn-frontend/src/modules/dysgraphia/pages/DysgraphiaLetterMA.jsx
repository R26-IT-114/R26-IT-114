...
import '../styles/dysgraphia-letter-ma.css';
...
// Stroke: CW circle-top → rightward lobe → inner spiral → sweep up → end(176.1,120)
const MA_GUIDE_PATH =
...
const START_MARKER = { x: 319.3, y: 180.0 };
const END_MARKER   = { x: 176.1, y: 120.0 };
...
const DysgraphiaLetterMA = () => {
  ...
  const handleAudio = () => {
    ...
    const u = new SpeechSynthesisUtterance('ම'); u.lang = 'si-LK';
 ...
 setEasyMode(false); attemptCountRef.current = 0;
  ...
  const submitCanvasForEvaluation = async () => {
    ...
    try {
      ...
      setEvalError(err?.response?.data?.error?.message || err.message || 'Evaluation failed');
    } finally {
  ...
  // ════════════════════════════════════════════════════════════════════════
  return (
    <main className='dg-shell dg-theme-ma'>
      <SpaceBackground />
      ...
      <section className='dg-stage'>
        <header className='dg-header'>
          <h1 onClick={handleAudio}>'ම' අක්ෂරය හුරු කරමු</h1>
        </header>
              ...
              {!blindMode && (
                <>
                  {!practiceBlind && !thirdPreviewVisible && (
                    <path d={MA_GUIDE_PATH} className='dg-chain-path' style={{ stroke: '#ffffff', strokeOpacity: 0.9, filter: 'drop-shadow(0 0 8px #ffffff)' }} />
                  )}
                  <path d={MA_GUIDE_PATH} ref={letterPathRef} style={{ stroke: 'none', fill: 'none' }} />
                  ...
                  {thirdPreviewVisible && (
                    <path d={MA_GUIDE_PATH} fill='none' stroke='#ffffff' strokeWidth='40'
                  ...
                  <div key='cheer' className='dg-cheer-overlay'>
                    ...
                    <div className="mt-5 px-8 py-4 rounded-3xl bg-black/40 backdrop-blur-md border border-yellow-400/40 shadow-2xl text-center">
                      ...
                      <p className="mt-2 text-lg font-bold text-white/90 tracking-wide">
                        ඔබ <span className="text-yellow-300">&quot;ම&quot;</span> අක්ෂරය නිවැරදිව ඇන්දා!
                      </p>
    ...
    setFreeTraceComplete(false);
        ...
        {drawingMode && !drawSuccess && (
          <div className='dg-draw-instruction'>
            {practiceBlind ? '✍️ දැන් "ම" අක්ෂරය ඔබම අඳින්න.' : '💧 තරු අනුපිළිවෙලට ඇඟිල්ල ගෙනයන්න '}
          </div>
        )}
        {showSuccessMessage && (
...
export default DysgraphiaLetterMA;