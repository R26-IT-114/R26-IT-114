...
import '../styles/dysgraphia-letter-ha.css';
...
const START_MARKER = { x: 101.9, y: 180.0 };
const END_MARKER = { x: 452.7, y: 420.0 };
...
// ──────────────────────────────────────────────────────
const HA_STAR_COLORS = ['#ffffff','#ffe4b5','#add8e6','#ffcccb','#b0e0e6','#fff176','#e0b0ff'];
const HaStarField = () => {
  const stars = Array.from({ length: 160 }, (_, i) => ({
    ...
    color: HA_STAR_COLORS[Math.floor(Math.random() * HA_STAR_COLORS.length)],
  }));
  return (
...
const DysgraphiaLetterHA = () => {
  ...
  const submitCanvasForEvaluation = async () => {
    ...
    try {
      ...
      setEvalResult(data);
      // Validate: check if the returned Sinhala letter matches "හ"
      const isCorrect = data?.predictions?.[0]?.sinhala === "හ" || data?.prediction?.sinhala === "හ";
  ...
  // ---------- Render ----------
  return (
    <main className='dg-shell dg-theme-ha'>
      <HaSpaceBackground />
