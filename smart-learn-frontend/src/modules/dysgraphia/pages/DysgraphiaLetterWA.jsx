...
import Topic from '../../../assets/images/dysgraphia/watopic.png';
...
// Stroke: body C-segs → L → reversed connector → CW circle, END(227.4,180)
const WA_GUIDE_PATH =
  'M 181.3 120.0 C 199.5 96.4 234.8 60.0 313.0 60.0 C 385.4 60.0 458.7 120.0 458.7 240.0 C 458.7 373.7 386.9 420.0 311.7 420.0 C 236.4 420.0 181.3 382.6 181.3 323.2 C 181.3 275.4 221.1 240.0 291.4 240.0 L 317.4 240.0 C 317.4 240.0 317.4 180.0 227.4 180.0 A 30 30 0 1 1 227.4 240.0 A 30 30 0 1 1 227.4 180.0';

const START_MARKER = { x: 181.3, y: 120.0 };
const END_MARKER = { x: 227.4, y: 180.0 };
...
const DysgraphiaLetterWA = () => {
  ...
  const handleAudio = () => {
    ...
    const u = new SpeechSynthesisUtterance('ව'); u.lang = 'si-LK';
      ...
      <section className='dg-stage'>
              ...
              {!blindMode && (
                <>
                  {!practiceBlind && !thirdPreviewVisible && (
                    <path d={WA_GUIDE_PATH} className='dg-chain-path' style={{ stroke: '#ffffff', strokeOpacity: 0.9, filter: 'drop-shadow(0 0 8px #ffffff)' }} />
                  )}
                  <path d={WA_GUIDE_PATH} ref={letterPathRef} style={{ stroke: 'none', fill: 'none' }} />
                  ...
                  {animationComplete && showGuide && !drawingMode && (
                    <g>
                      <defs>
                        <linearGradient id='wa-done-rainbow' x1='181' y1='120' x2='227' y2='180' gradientUnits='userSpaceOnUse'>
                        ...
                        <filter id='wa-done-glow' x='-30%' y='-30%' width='160%' height='160%'>
                          <feGaussianBlur stdDeviation='7' result='blur' />
                          <feMerge><feMergeNode in='blur' /><feMergeNode in='SourceGraphic' /></feMerge>
                        </filter>
                      </defs>
                      <path d={WA_GUIDE_PATH} fill='none' stroke='rgba(200,130,255,0.40)' strokeWidth='56' strokeLinecap='round' filter='url(#wa-done-glow)' />
                      <path d={WA_GUIDE_PATH} fill='none' stroke='url(#wa-done-rainbow)' strokeWidth='34' strokeLinecap='round' />
                      <path d={WA_GUIDE_PATH} fill='none' stroke='rgba(255,255,255,0.65)' strokeWidth='14' strokeLinecap='round' />
                      <path d={WA_GUIDE_PATH} fill='none' stroke='#ffea00' strokeWidth='8' strokeLinecap='round' strokeDasharray='0 22' opacity='0.95' style={{ filter: 'drop-shadow(0 0 6px #ffea00)' }} />
                      <path d={WA_GUIDE_PATH} fill='none' stroke='#ff4081' strokeWidth='6' strokeLinecap='round' strokeDasharray='0 16' strokeDashoffset='8' opacity='0.90' style={{ filter: 'drop-shadow(0 0 5px #ff4081)' }} />
                      <path d={WA_GUIDE_PATH} fill='none' stroke='#40c4ff' strokeWidth='5' strokeLinecap='round' strokeDasharray='0 12' strokeDashoffset='4' opacity='0.85' style={{ filter: 'drop-shadow(0 0 5px #40c4ff)' }} />
                      <path d={WA_GUIDE_PATH} fill='none' stroke='#69f0ae' strokeWidth='4' strokeLinecap='round' strokeDasharray='0 9' strokeDashoffset='2' opacity='0.80' style={{ filter: 'drop-shadow(0 0 4px #69f0ae)' }} />
                    </g>
                  )}

                  {/* ── Third star preview flash ── */}
                  {thirdPreviewVisible && (
                    <path d={WA_GUIDE_PATH} fill='none' stroke='#ffffff' strokeWidth='40'
        ...
        {drawingMode && !drawSuccess && (
          <div className='dg-draw-instruction'>
            {practiceBlind ? '✍️ දැන් "ව" අක්ෂරය ඔබම අඳින්න.' : '💧 තරු අනුපිළිවෙලට ඇඟිල්ල ගෙනයන්න '}
          </div>
        )}
        {showSuccessMessage && (
