...
const ListenCard = ({ word, onSpeak, isSpeaking }) => (
  <div className="bg-white/88 backdrop-blur-sm rounded-[32px] shadow-xl
    ...
    {/* Show word text below speaker so child can see it too */}
    <div className="mt-4 inline-block bg-[#F0FAF4] border-2 border-[#A8D5BA] rounded-2xl px-6 py-2">
      <span
...
const ImageChoice = ({ item, cardState, onSelect, disabled }) => (
  ...
  >
    ...
    {cardState === 'correct' && (
      ...
      >
        <Check size={44} className="text-white drop-shadow-md" strokeWidth={3} />
      </motion.div>
    )}
    ...
    {cardState === 'wrong' && (
      <motion.div
        ...
        animate={{ opacity: 1 }}
      >
        <X size={44} className="text-white drop-shadow-md" strokeWidth={3} />
      </motion.div>
    )}
  </motion.button>
);
...
const TwoLetterListenMatch = () => {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => { setSelectedId(null); setPhase('choosing'); }, 900);
    return () => clearTimeout(t);
  }, [phase]);
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-center">
            <p className="text-[#2D6A4A] font-semibold text-sm flex items-center justify-center gap-1">
              <Headphones size={14} strokeWidth={2} /> අකුරු දෙකේ වචන
            </p>
        ...
        {phase === 'finished' ? (
          ...
          <AnimatePresence mode="wait">
            <IntroCard
              key="intro"
              icon={Headphones}
              title="අකුරු දෙකේ වචන"
              instruction="ශබ්දය හොඳින් අසා, ශ්‍රවණය කළ වචනයට අදාල රුපය ස්පර්ශ කරන්න!"
              level={level}
              total={questions.length}
              onStart={handleStart}
            />
...
export default TwoLetterListenMatch;