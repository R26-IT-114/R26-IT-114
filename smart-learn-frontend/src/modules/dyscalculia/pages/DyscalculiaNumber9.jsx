...
const START_MARKER = { x: 410, y: 250 };
...
const AUDIO_TEXT = 'නවය';

const NUMBER_GUIDE_PATH = 'M 410 250 C 410 155 310 120 245 175 C 190 220 210 330 285 355 C 360 380 430 330 420 250 C 420 365 405 455 330 500';
...
const DyscalculiaNumber9 = () => {
  ...
  const { level, levelCompletion, savePrediction, goToLevelSelection } = useNumberTracingProgress(9);
  ...
  const submitCanvasForEvaluation = async () => {
    ...
    try {
      ...
      if (result?.isCorrect === true) {
        ...
        setEvalError(
          `Model detected: ${detectedNumber}. Please try drawing 9 again.`
        );
      }
    } catch (error) {
    ...
    >
      ...
      <section className='dg-stage dc-trace-stage'>
        <header className='dg-header dc-instruction-box'>
          <h1 onClick={handleAudio}>‘9’ {AUDIO_TEXT} අංකය ලියමු</h1>
        </header>
        ...
        <div className='dg-canvas-wrap dc-trace-card'>
          {!drawingWithCanvas ? (
            ...
            <div className='dg-practice-wrap' style={{ width: '100%', height: '100%' }}>
              {/* <h3>✍️ {AUDIO_TEXT} “9” අංකය අඳින්න</h3> */}
              <div
