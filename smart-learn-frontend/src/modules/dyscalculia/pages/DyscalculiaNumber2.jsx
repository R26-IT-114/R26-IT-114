...
const START_MARKER = { x: 220, y: 190 };
...
const AUDIO_TEXT = 'දෙක';

const NUMBER_GUIDE_PATH = 'M 220 190 C 245 130 390 120 425 190 C 460 260 395 320 330 375 C 285 415 245 455 210 500 L 450 500';
...
const DyscalculiaNumber2 = () => {
  const navigate = useNavigate();
  const { level, levelCompletion, savePrediction, goToLevelSelection } = useNumberTracingProgress(2);
  ...
  const submitCanvasForEvaluation = async () => {
    ...
    try {
      ...
      if (result?.isCorrect === true) {
        ...
        setEvalError(
          `Model detected: ${detectedNumber}. Please try drawing 2 again.`
        );
      }
    } catch (error) {
    ...
    >
      ...
      <section className='dg-stage dc-trace-stage'>
        <header className='dg-header dc-instruction-box'>
          <h1 onClick={handleAudio}>‘2’ {AUDIO_TEXT} අංකය ලියමු</h1>
        </header>
        ...
        <div className='dg-canvas-wrap dc-trace-card'>
          {!drawingWithCanvas ? (
            ...
            <div className='dg-practice-wrap' style={{ width: '100%', height: '100%' }}>
              {/* <h3>✍️ {AUDIO_TEXT} “2” අංකය අඳින්න</h3> */}
              <div
...
export default DyscalculiaNumber2;
