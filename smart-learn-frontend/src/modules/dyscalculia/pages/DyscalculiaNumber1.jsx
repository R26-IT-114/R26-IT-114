...
const START_MARKER = { x: 285, y: 165 };
...
const [evalResult, setEvalResult] = useState(null);
  ...
  const drawingStrokeColor = useMemo(() => {
    ...
    if (t <= 0.5) {
      return mixHexColors('#ff6b9d', '#ffca28', t / 0.5);  // pink → amber
    }
    return mixHexColors('#ffca28', '#2ed573', (t - 0.5) / 0.5);  // amber → green
    ...
    <main className='dg-shell dg-theme-ta dc-number-page dc-cartoon-bg'>
      ...
      <section className='dg-stage dc-trace-stage'>
        <header className='dg-header dc-instruction-box'>
          <h1 onClick={handleAudio}>‘1’ {AUDIO_TEXT} අංකය ලියමු</h1>
        </header>
        ...
        <div className='dg-canvas-wrap dc-trace-card'>
          {!drawingWithCanvas ? (
            ...
            <div className='dg-practice-wrap' style={{ width: '100%', height: '100%' }}>
              <h3>✍️ {AUDIO_TEXT} “1” අංකය අඳින්න</h3>
              <div className='dg-practice-canvas-shell' style={{ position: 'relative', width: 600, height: 600, margin: '16px auto', borderRadius: '20px', overflow: 'hidden' }}>
                <ReactSketchCanvas
                  ref={canvasRef}
                  width='600px'
                  height='600px'
                  ...
                  style={{ border: 'none', borderRadius: '20px', position: 'absolute', top: 0, left: 0, cursor: PEN_CURSOR }}
                />
              </div>
...
=======
                <button type='button' className='dg-practice-clear-btn dg-ctl-btn' onClick={() => canvasRef.current?.clearCanvas()} style={{ color: '#ffffff' }}>
                  🧹 පිරිසිදු කරමු
>>>>>>> ff9b4ed09de51e685175c407d99714a4c0c58115
                </button>
        ...
        {drawingMode && !drawSuccess && (
          <div className='dg-draw-instruction'>
            {practiceBlind ? '👉 අංකය “1” මතක තියා අඳින්න.' : '✒️ මඟ පෙන්වූ රේඛාව දිගේ අංකය අඳින්න.'}
          </div>
        )}
