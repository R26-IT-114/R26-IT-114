...
const START_MARKER = { x: 300, y: 150 };
...
const NUMBER_GUIDE_PATH = 'M 300 150 L 340 120 L 340 500';
...
const DyscalculiaNumber1 = () => {
  ...
  const { level, levelCompletion, savePrediction, goToLevelSelection } = useNumberTracingProgress(1);
  ...
  const submitCanvasForEvaluation = async () => {
    ...
    try {
      ...
      if (result?.isCorrect === true) {
        ...
        setEvalError(
          `Model detected: ${detectedNumber}. Please try drawing 1 again.`
        );
      }
    } catch (error) {
