...
import '../../dysgraphia/styles/dysgraphia-common.css';
import '../../dysgraphia/styles/dysgraphia-letter-ta.css';
...
const DyscalculiaNumberTemplate = ({ digit, audioText, numberGuidePath, startMarker }) => {
  ...
  const [markerPosition, setMarkerPosition] = useState(startMarker);
  const [showGuide, setShowGuide] = useState(false);
  const [animatePop, setAnimatePop] = useState(false);
  const [animationComplete, setAnimationComplete] = useState(false);
  const [nodesDeployed, setNodesDeployed] = useState(false);
  const [originPoint, setOriginPoint] = useState({ x: -100, y: startMarker.y });
    ...
    const animate = (now) => {
      ...
      if (nextProgress >= 1) {
        ...
        if (pathElement) {
          ...
          for (let i = 0; i < 6; i += 1) setTimeout(() => playBubbleSound(), i * 80);
        }
        return;
      }
      const pathElement = letterPathRef.current;
  ...
  const finalStrokeWidth = drawingMode
    ? Math.min(52, 20 + overallProgress * 18 + (isDrawing ? 6 : 0))
    : 22;
        ...
        <div className='dg-canvas-wrap'>
          {!drawingWithCanvas ? (
            ...
            >
              ...
              {!blindMode && (
                <>
                  ...
                  <path d={numberGuidePath} ref={letterPathRef} style={{ stroke: 'none', fill: 'none' }} />
                  ...
                  {showGuide && !drawingMode && (
                    <>
                      <circle
                        cx={nodesDeployed ? startMarker.x : originPoint.x}
                        cy={nodesDeployed ? startMarker.y : originPoint.y}
                        r='22'
                        className={`dg-node ${nodesDeployed ? 'dg-deployed' : ''}`}
                      />
                      <text
                        x={nodesDeployed ? startMarker.x : originPoint.x}
                        y={nodesDeployed ? startMarker.y + 6 : originPoint.y + 6}
                        textAnchor='middle'
                      >
                        ★
                      </text>
                      <circle cx={markerPosition.x} cy={markerPosition.y} r='0' style={{ opacity: 0 }} />
                    </>
                  )}
            ...
            <div className='dg-practice-wrap' style={{ width: '100%', height: '100%' }}>
              <h3>✍️ {audioText} “{digit}” අංකය අඳින්න</h3>
              <div
