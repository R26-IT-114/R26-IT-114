...
// Number data mapping for 0-9
const NUMBER_DATA = {
  0: {
    path: 'M 320 120 C 230 120 175 205 175 310 C 175 430 235 505 320 505 C 405 505 465 430 465 310 C 465 205 410 120 320 120',
    startMarker: { x: 320, y: 120 },
    audioText: 'බිංදුව'
  },
  1: {
    path: 'M 285 165 L 345 120 L 345 500 M 270 500 L 420 500',
    startMarker: { x: 285, y: 165 },
    audioText: 'එක'
  },
  2: {
    path: 'M 225 190 C 250 125 385 115 425 185 C 470 265 395 325 330 380 C 285 420 245 455 220 500 L 455 500',
    startMarker: { x: 225, y: 190 },
    audioText: 'දෙක'
  },
  3: {
    path: 'M 220 165 C 285 115 435 125 435 225 C 435 285 380 315 325 315 C 400 315 460 350 460 425 C 460 525 305 545 215 480',
    startMarker: { x: 220, y: 165 },
    audioText: 'තුන'
  },
  4: {
    path: 'M 420 120 L 215 375 L 470 375 M 420 120 L 420 505',
    startMarker: { x: 420, y: 120 },
    audioText: 'හතර'
  },
  5: {
    path: 'M 430 130 L 245 130 L 225 295 C 275 265 370 275 420 330 C 480 395 435 510 325 510 C 265 510 225 480 205 445',
    startMarker: { x: 430, y: 130 },
    audioText: 'පහ'
  },
  6: {
    path: 'M 420 155 C 320 105 215 200 210 345 C 205 470 300 535 390 490 C 470 450 460 340 385 315 C 310 290 230 325 220 395',
    startMarker: { x: 420, y: 155 },
    audioText: 'හය'
  },
  7: {
    path: 'M 205 130 L 465 130 L 315 505',
    startMarker: { x: 205, y: 130 },
    audioText: 'හත'
  },
  8: {
    path: 'M 320 130 C 250 130 210 180 215 245 C 220 300 270 330 320 330 C 390 330 430 285 425 230 C 420 165 380 130 320 130 C 250 130 210 180 215 245 C 220 305 270 335 320 335 C 390 335 435 385 435 445 C 435 520 375 550 320 550 C 250 550 205 500 210 435 C 215 375 265 335 320 335',
    startMarker: { x: 320, y: 130 },
    audioText: 'අට'
  },
  9: {
    path: 'M 420 245 C 420 155 315 115 245 175 C 185 225 210 335 285 360 C 365 388 435 330 420 245 C 420 360 390 455 300 510',
    startMarker: { x: 420, y: 245 },
    audioText: 'නවය'
  }
};

// Simple "{targetNumber}" trace path in viewBox 0 0 640 600, continuous for getPointAtLength.
const NUMBER_GUIDE_PATH =
  'M 320 120 C 230 120 175 205 175 310 C 175 430 235 505 320 505 C 405 505 465 430 465 310 C 465 205 410 120 320 120';

const DyscalculiaNumber = () => {
  const navigate = useNavigate();
  const { number } = useParams();
  const targetNumber = parseInt(number);

  // Get number data or default to 0
  const numberData = NUMBER_DATA[targetNumber] || NUMBER_DATA[0];
  const NUMBER_GUIDE_PATH = numberData.path;
  const START_MARKER = numberData.startMarker;
  const AUDIO_TEXT = numberData.audioText;
      ...
      <section className='dg-stage dc-trace-stage'>
        ...
        {drawingMode && !drawSuccess && (
          <div className='dg-draw-instruction'>
            {practiceBlind
              ? '👉 අංකය "{targetNumber}" මතක තියා අඳින්න.'
