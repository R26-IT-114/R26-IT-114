...
const START_MARKER = { x: 320, y: 130 };
...
const NUMBER_GUIDE_PATH = 'M 320 130 C 250 130 215 180 215 245 C 215 310 270 335 320 335 C 390 335 430 385 430 445 C 430 515 370 545 320 545 C 250 545 210 495 210 435 C 210 370 260 335 320 335 C 390 335 425 285 425 230 C 425 165 380 130 320 130';
...
const DyscalculiaNumber8 = () => {
  const navigate = useNavigate();
  const { level, levelCompletion, savePrediction, goToLevelSelection } = useNumberTracingProgress(8);
  ...
  const submitCanvasForEvaluation = async () => {
    ...
    try {
      ...
      if (result?.isCorrect === true) {
        ...
        setEvalError(
          `Model detected: ${detectedNumber}. Please try drawing 8 again.`
        );
      }
    } catch (error) {
    ...
    >
      ...
      <section className='dg-stage dc-trace-stage'>
        <header className='dg-header dc-instruction-box'>
          <h1 onClick={handleAudio}>‘8’ {AUDIO_TEXT} අංකය ලියමු</h1>
        </header>
        ...
        <div className='dg-canvas-wrap dc-trace-card'>
          {!drawingWithCanvas ? (
            ...
            <div className='dg-practice-wrap' style={{ width: '100%', height: '100%' }}>
              {/* <h3>✍️ {AUDIO_TEXT} “8” අංකය අඳින්න</h3> */}
              <div
