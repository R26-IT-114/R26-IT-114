...
const START_MARKER = { x: 220, y: 160 };
...
const AUDIO_TEXT = 'තුන';

const NUMBER_GUIDE_PATH = 'M 220 160 C 290 110 430 130 430 230 C 430 290 375 315 320 315 C 390 315 455 350 455 425 C 455 520 310 540 220 480';
...
const DyscalculiaNumber3 = () => {
  const navigate = useNavigate();
  const { level, levelCompletion, savePrediction, goToLevelSelection } = useNumberTracingProgress(3);
  ...
  const submitCanvasForEvaluation = async () => {
    ...
    try {
      ...
      if (result?.isCorrect === true) {
        ...
        setEvalError(
          `Model detected: ${detectedNumber}. Please try drawing 3 again.`
        );
      }
    } catch (error) {
    ...
    >
      ...
      <section className='dg-stage dc-trace-stage'>
        <header className='dg-header dc-instruction-box'>
          <h1 onClick={handleAudio}>‘3’ {AUDIO_TEXT} අංකය ලියමු</h1>
        </header>
        ...
        <div className='dg-canvas-wrap dc-trace-card'>
          {!drawingWithCanvas ? (
            ...
            <div className='dg-practice-wrap' style={{ width: '100%', height: '100%' }}>
              {/* <h3>✍️ {AUDIO_TEXT} “3” අංකය අඳින්න</h3> */}
              <div
...
export default DyscalculiaNumber3;
